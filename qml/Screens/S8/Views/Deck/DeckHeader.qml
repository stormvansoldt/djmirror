...
Item {
  ...
  Widgets.SpinningWheel {
      ...
      Text {
          ...
          font.pixelSize: fonts.scale((loopSizePos < 5) ? fonts.miniFontSize : fonts.miniFontSizePlusPlus);
          font.family: prefs.fontDeckHeaderString
  ...
  Item {
      ...
      Rectangle {
          ...
          property real bpmDifference: {
              ...
              const currentBpm = propMixerBpm.value
              const masterBpm = propMixerStableBpm.value
      ...
      Widgets.PhaseMeter {
          ...
          width: parent.width - ((master_indicator.width + 2)* 4)
          height: prefs.phaseMeterHeight
          // height: parent.height
  ...
  function updateCoverArt() {
      if (headerState == "small" || deckType == DeckType.Live || directThru.value) {
          ...
          cover_small.anchors.leftMargin = 2; // Mantener el margen incluso cuando está oculto
          ...
          cover_small.anchors.leftMargin = 2; // Mantener el margen de 2 pixels
