...
Item {
  ...
  readonly property int   maxTextWidth:   150 * screenScale    // Escalar el ancho máximo de texto
  readonly property int   arrowContainerWidth: 18 * screenScale // Escalar el contenedor de flechas
  ...
  Text {
    ...
    font.pixelSize: header.fontSize * screenScale
  ...
  Item {
   id: textContainter
    ...
    anchors.rightMargin: spaceToDeckLetter * screenScale
    clip: false // true
    ...
    Flow {
      ...
      Repeater {
        ...
        Item {
          ...
          property int pathLevel: stringList.length - index - 1
          
          width: headerPath.width + arrowContainerWidth
          height: 20 * screenScale  // Escalar la altura del contenedor de texto
          ...
          Widgets.TextSeparatorArrow {
            ...
            anchors.topMargin:   4 * screenScale  // Escalar el margen superior
            anchors.rightMargin: 6 * screenScale  // Escalar el margen derecho
          }
          ...
          // Añadir MouseArea para que cada elemento de la ruta sea tocable
          MouseArea {
            anchors.fill: parent
            enabled: index > 0 // No permitir tocar en el elemento actual
            
            onClicked: {
              // Calcular cuántos niveles debemos retroceder
              var levelsToGoUp = index;
              
              // Salir de los nodos correspondientes
              for (var i = 0; i < levelsToGoUp; i++) {
                qmlBrowser.exitNode = true;
                // Pequeña pausa para que la navegación funcione correctamente
                navTimer.start();
              }
            }
          }
  ...
  // Timer para secuenciar la navegación entre niveles
  Timer {
    id: navTimer
    interval: 100
    repeat: false
  ...
  Rectangle {
    ...
    height:        2 * screenScale  // Escalar el borde negro
   ...
   Rectangle {    
    ...
    height:        3 * screenScale  // Escalar el gradiente
    gradient: Gradient {
  ...
  states: [
    State {
      name: "show"
      PropertyChanges { target: header; height: 17 * screenScale }
    },
    State {
      name: "hide"
