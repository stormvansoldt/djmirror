...
Item {
  ...
  Row {
    ...
    spacing:              (sizeState == "small") ? (6 * screenScale) : (8 * screenScale)  // Reducir espaciado
    anchors.leftMargin:   (sizeState == "small") ? (8 * screenScale) : (2 * screenScale)  // Reducir márgenes
    anchors.rightMargin:  (sizeState == "small") ? (8 * screenScale) : (2 * screenScale)  // Reducir márgenes
    anchors.topMargin:    (sizeState == "small") ? (1 * screenScale) : (4 * screenScale)  // Ajustar margen superior
    ...
    Repeater {  // this repeater creates the four columns of the remix deck
      ...
      RemixDeckColumn {
        ...
        height: parent.height; 
        width: parent.width / 4 - (parent.spacing * 0.75)  // Calcular ancho dinámicamente
        state: sizeState; 
