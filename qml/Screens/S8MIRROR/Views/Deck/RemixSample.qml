...
Item {
  ...
  width: parent.width
  height: 55 * screenScale  // Aumentar altura del sample individual
  ...
  AppProperty { id: tigger;      path: samplePropertyPath + ".trigger" } 
  AppProperty { id: stop;        path: samplePropertyPath + ".stop" } 
  ...
  Rectangle {
    ...
    MouseArea {
      anchors.fill: parent
      onClicked: {
         if (playState.description !== "Empty") {
          // Si la muestra está reproduciéndose, la detenemos
          if (playState.description === "Playing") {
            stop.value = 1
          } else {
            // Si la muestra está cargada pero no reproduciéndose, la reproducimos
            tigger.value = 1
          }
        }
      }
    ...
    Image {
      ...
      anchors.topMargin:  3 * screenScale   // Escalar margen
      ...
      visible:            false
      source: "../Images/Remix_Sample_Icon_Loop" + colors.inverted + ".png"
      state:              playMode.description 
      states: [
        State { name: "Looped";  PropertyChanges { target: remixSampleIcon; source: "../Images/Remix_Sample_Icon_Loop" + colors.inverted + ".png"    } },
        State { name: "OneShot"; PropertyChanges { target: remixSampleIcon; source: "../Images/Remix_Sample_Icon_OneShot" + colors.inverted + ".png" } } 
      ]
    }
    ...
    Text {
      ...
      anchors.leftMargin: 24 * screenScale
      anchors.rightMargin: 20 * screenScale
