...
Rectangle {
    ...
    Rectangle {
        ...
        width: 3 * screenScale  // Escalar ancho
        height: parent.height + (2 * screenScale)  // Escalar altura
        color: colors.colorRedPlaymarker
        visible: trackIsLoaded
        x: (trackLength.value != 0) ? waveform.width * (playPosition.value / trackLength.value) - (1 * screenScale) : 0  // Escalar offset
        anchors.verticalCenter: parent.verticalCenter

        border {
            color: colors.colorBlack50
            width: 1 * screenScale  // Escalar ancho del borde
        }
    }
