...
Item {
  ...
  readonly property int waveformHeight: (deckSizeState == "small") ? 0 : ( parent ? ( (deckSizeState == "medium") ? (parent.height-55 * screenScale) : (parent.height-70 * screenScale) ) : 0 )

  readonly property int largeDeckBottomMargin: (waveformContainer.isStemStyleDeck) ? 1 * screenScale : 1 * screenScale
  readonly property int smallDeckBottomMargin: (deckId > 1) ? 1 * screenScale : 1 * screenScale
  ...
  WF.WaveformContainer {
    ...
    anchors.bottomMargin: {
      ...
      return (incMargin) + (5 * screenScale)
    }
  ...
  Row {
    id: hotcues
    height: prefs.displayHotCueBar && deckSizeState != "small" ? 19 * screenScale : 0
    visible: prefs.displayHotCueBar && deckSizeState != "small" && trackIsLoaded ? true : false
    spacing: 2 * screenScale
    anchors.bottom: parent.bottom
    anchors.bottomMargin: deckSizeState == "medium" ? 1 * screenScale : 0
    anchors.left: parent.left
    anchors.right: parent.right
    ...
    Repeater {
      ...
      Rectangle {
        ...
        AppProperty { id: active;  path: "app.traktor.decks." + (deckId+1) + ".track.cue.hotcues." + (index + 1) + ".active" }

        AppProperty { id: play;  path: "app.traktor.decks." + (deckId+1) + ".play" }

        width: (parent.width - (14 * screenScale)) / 8
        height: parent.height
        color: exists.value > 0 ? hotcueColors[type.value] : colors.colorBgEmpty

        // MouseArea para lanzar hotcues
        MouseArea {
          anchors.fill: parent
          enabled: exists.value > 0
          onClicked: {
            active.value = 0
            play.value = 1
            active.value = 1
