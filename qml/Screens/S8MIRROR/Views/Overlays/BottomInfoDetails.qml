...
Rectangle {
  ...
  // Señales para notificar cambios en las propiedades principales
  signal parameterValueChanged()
  signal activeValueChanged()
  signal nameValueChanged()
  ...
  AppProperty { 
    id: parameter
    path: playerPath + ".filter_value"
    onValueChanged: parameterValueChanged()
  }
  AppProperty { 
    id: active
    path: playerPath + ".filter_on"
    onValueChanged: activeValueChanged()
  } 
  AppProperty { 
    id: name
    path: fxUnitPath + ".name"
    onValueChanged: nameValueChanged()
  }
  AppProperty { id: fxButtonName;    path: fxUnitPath + ".buttons." + column + ".name" }
  AppProperty { 
    id: fxUnitMode
    path: fxUnitPath + ".type"
    onValueChanged: parameterValueChanged()
  }
  AppProperty { 
    id: fxSelect1
    path: fxUnitPath + ".select.1"
    onValueChanged: parameterValueChanged()
  }
  AppProperty { 
    id: fxColumnSelect
    path: fxUnitPath + ".select." + column
    onValueChanged: parameterValueChanged()
  }
  ...
  // Añadir propiedad para calcular el ancho proporcional
  property real sliceWidth: (width - (16 * screenScale)) // Ancho total menos márgenes (8 por cada lado)
  ...
  Item {
    ...
    width: parent.width
    height: parent.height - (bottomInfoDetails.sliderHeight) - (3 * screenScale)
    clip: false // true
    ...
    Text {
      ...
      anchors.top: parent.top
      anchors.left: parent.left
      anchors.topMargin: (sizeState == "large") ? (21 * screenScale) : (2 * screenScale)
      anchors.leftMargin: (8 * screenScale) + ((column > 1 && sizeState != "large") ? (sliceWidth - width) : 0)
    ...
    Item {
      anchors.top: parent.top
      anchors.topMargin: 40 * screenScale
      anchors.left: parent.left
    ...
    Rectangle {
      ...
      anchors.top: parent.top
      anchors.topMargin: 42 * screenScale
      width: 30 * screenScale
      height: 15 * screenScale
      color: isEnabled && (bottomInfoFilterButtonText.text != "") ? bottomInfoDetails.colorButtonEnabled : bottomInfoDetails.colorButtonDisabled
  ...
  Rectangle {
    ...
    readonly property int dividerHeight: 57 * screenScale
    ...
    visible: !((column == 0 || column == 3) && bottomInfoDetails.state == "FX")
  }
  ...
  // Cuando cambia el estado de control, actualizar las conexiones de señales
  onStateChanged: {
    // Restaurar las señales para que funcionen con los nuevos paths
    if (parameter && parameter.path) parameterValueChanged()
    if (active && active.path) activeValueChanged()
    if (name && name.path) nameValueChanged()
  }
  ...
  states: [
    State {
      ...
      PropertyChanges { target: valueString;                font.pixelSize: fonts.largeFontSizePlus * screenScale }
    ...
    State {
      ...
      PropertyChanges { target: valueString;                toPercent: false                                         }
      PropertyChanges { target: valueString;                font.pixelSize: fonts.middleFontSize * screenScale       }
      PropertyChanges { target: active;                     path:      ""                                            }
      PropertyChanges { target: name;                       path:      namePath                                      }
