...
Traktor.Beatgrid {
  ...
  Traktor.WaveformTranslator {
    ...
    Traktor.BeatgridLines
    {
      ...
      height: prefs.fullHeightBeatgridLines ? parent.height : (prefs.heightBeatgridLines * screenScale)

      beatMarkerList: beatMarkers
      color: prefs.fullHeightBeatgridLines ? colors.colorWhite25 : colors.colorWhite75
      width: 1 * screenScale
    }

    Traktor.BeatgridLines
    {
      visible: !prefs.fullHeightBeatgridLines
      anchors.bottom: parent.bottom
      anchors.left: parent.left
      anchors.right: parent.right
      height: prefs.heightBeatgridLines * screenScale
    ...
    Traktor.BeatgridLines
    {
      ...
      anchors.left: parent.left
      anchors.right: parent.right
      height: prefs.fullHeightRedBeatgridLines ? parent.height : (prefs.heightRedBeatgridLines * screenScale)

      beatMarkerList: barMarkers
      color: prefs.fullHeightRedBeatgridLines ? colors.colorRedPlaymarker75 : colors.colorRedPlaymarker
      width: 1 * screenScale
    }

    Traktor.BeatgridLines
    {
      visible: !prefs.fullHeightRedBeatgridLines
      anchors.bottom: parent.bottom
      anchors.left: parent.left
      anchors.right: parent.right
      height: prefs.heightRedBeatgridLines * screenScale
  ...
  Repeater {
    ...
    Rectangle {
      ...
      width: overlayWidth
      x: (index == 0) ? 0 : view.width - overlayWidth + (1 * screenScale)
    }
  }
  ...
  Repeater { 
    ...
    Item {
      ...
      width: 1 * screenScale
      visible: view.editEnabled && !isAnalyzing
      x: overlayWidth + index * (beatWidth + (1 * screenScale))
      ...
      Rectangle {
        ...
        y: 0
        width: 1 * screenScale
        height: view.height - (2 * screenScale)
